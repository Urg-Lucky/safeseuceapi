...
if ($c_img && $ad_pdf && $pan_pdf) {
    ...
    if (
        ...
        $insertQuery = $con->prepare("INSERT INTO Candidate (name, address, mobile, ad_num, pan_num, login, c_img, ad_pdf, pan_pdf, status, Pwd, bid,hname,sname,Ac_num)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

        $insertQuery->bind_param("sssssssssssssss", $name, $address, $mobile, $ad_num, $pan_num, $login, $targetFile1, $targetFile2, $targetFile3, $status, $Pwd, $bid, $hname, $sname, $Ac_num);

        if ($insertQuery->execute()) {
            $response = array(
                "status" => "success",
                "message" => "Data inserted & Files uploaded successfully",
            );
            http_response_code(201);
        } else {
            $response = array(
                "status" => "failure",
                "message" => "Error inserting data into Candidate table: " . $insertQuery->error,
            );
    ...
    $response = array(
        "status" => "failure",
        "message" => "Required files not uploaded.",
    );
